...
(function (root, factory) {
	...
	function durationFormat() {
		...
		var tokenizer, tokens, types, typeMap, momentTypes, foundFirst, reachedLargest, trimIndex,
		...
		tokens = map(tokens, function (token, index) {
            ...
            if (settings.largest === 0) {
                reachedLargest = true;
                return "";
            }

            // decrement "largest" option for each moment token
            // if this value reaches `0`, all subsequent tokens will be ignored
            settings.largest -= 1;
        ...
        // trim trailing comma, space, and colon when using the `largest` option
        // not compatible with `trim: right`
        if (settings.largest === 0) {
